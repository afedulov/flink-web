...
On the left side you can see a visual representation of financial transactions flowing through the system after you click the "Start" button. Slider at the top allows you to control the number of transactions per second generated for the demo purposes. The middle section is devoted to managing the rules evaluated by Flink. In this section you can create new rules as well as issue certain control commands, such as clearing Flink's state.

The demo out of the box comes with a set of predefined sample rules. You can simply click the _Start_ button and after some time observe alerts displayed in the right section of the UI. These alerts are the result of Flink evaluating the generated transactions stream against the predefined rules.
...
Frontend -> Backend, relay all control commands....
 Backend includes a Transactions Generator, which sends emulated transactions to Flink via Kafka. Whenever a new rule definition is created in the UI, the backend also pushes it to Flink via a separate "Control" topic. Alerts generated by Flink are consumed by the Backend from "Alerts" topic and relayed to the UI via WebSockets.
...
Now that you know the layout of what our goal is, let's look into the details of what is required to implement such system, starting with looking at what
